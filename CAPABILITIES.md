...
| **nbb** | NikBearBrown cut — Teardown register, LLM exercise + dig-deeper (second-to-last), NikBearBrown outro (last); writes into `nbb-` directory (source never modified). **ElevenLabs only — GATE P before spend** | *(none for scaffold)* | ElevenLabs NIKBEARBROWN only (no Kokoro) | free (scaffold) → $$ (ElevenLabs paid clone) | `examples/explainer--size-paradox/` | source reel/lecture with `beat_sheet.json`; `AUTHOR.MD` NikBearBrown section for outro; `ELEVENLABS_VOICE_NIKBEARBROWN` in `.env` |
