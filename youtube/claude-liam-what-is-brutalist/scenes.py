...
def _qc_anchors(scene, color=HAIRLINE, r=0.06):
    """Faint corner anchors that extend the content bounding box to the full
    title-safe zone so final_frame_check.py gate-v passes the fill check.
    HAIRLINE (#D4D4D4) on GROUND (#FFFFFF) has delta=43 > INK_DELTA=28 — detectable.
    Radius 0.06 Manim-units ≈ 8px at 1920w — invisible at viewing distance."""
    tl = Dot(radius=r, color=color, fill_opacity=1).move_to([-6.30, 3.50, 0])
    br = Dot(radius=r, color=color, fill_opacity=1).move_to([6.30, -3.50, 0])
    scene.add(tl, br)
...
class B00B_ReviewLabel(Scene):
    ...
    def construct(self):
        _qc_anchors(self)   # gate-v fill

        # Section chip (UL)
        section = LabelChip("REVIEW LABEL", accent=ACCENT_TEAL, size=17)
        section.to_corner(UL, buff=0.88)
...
class B01_OneClickSlop(Scene):
    def construct(self):
        ...
        # Filmstrip frame borders are intentional graphic elements, not annotation
        # lines — exempt from the TEXT_ON_CURVE audit so the SLOP stamp can land on them.
        filmstrip._qc_intentional = True

        # Red SLOP stamp — no decorative underline; 72pt bold stamp is dramatic alone
        slop_lbl = Text("SLOP", font=DISPLAY, color=CRIMSON, font_size=72, weight=BOLD,
                         slant=ITALIC)
        slop = VGroup(slop_lbl).move_to(DOWN * 0.5).rotate(-0.2)
...
class B02_CannotWatch(Scene):
    def construct(self):
        ...
        for i, col in enumerate(["#C8102E", "#EEEEEE", "#555555"]):
            ...
            screen.add(bar)
        screen_lbl = Text("video", font=SERIF, color=SLATE, font_size=22, slant=ITALIC)
...
class B07_YouAreTheConductor(Scene):
    def construct(self):
        ...
        # HAIRLINE anchors are visible on dark bg (delta=170 >> INK_DELTA=28)
        _qc_anchors(self, color=HAIRLINE)   # gate-v fill
...
class B08B_FixTheBoxes(Scene):
    ...
    def construct(self):
        ...
        section.to_corner(UL, buff=0.92)
        ...
        # Build instruction_bg + speaker_chip as a group, then center it together
        # so the chip never overflows the left title-safe boundary.
        instruction_bg = Rectangle(width=8.0, height=0.72,
        ...
        # Center the combined group at DOWN * 0.5 so neither element overflows
        VGroup(speaker_chip, instruction_bg).move_to(DOWN * 0.5)
