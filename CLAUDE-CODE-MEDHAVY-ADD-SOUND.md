# Claude Code Prompt — Add Sound to Silent Medhavy Videos (Kokoro, free)

The Medhavy videos rendered SILENT — their narration mp3s were never generated, so the
audio-first pipeline composited silence. This generates the free Kokoro narration
(the reel's assigned Medhavy voice — Liam `am_onyx` or Bella af_bella), re-renders at 4K so the
sound composites in, and overwrites the silent copy in `TMP/medhavy`. Unattended, FREE
...
```text
Add sound to every silent Medhavy video. Unattended batch: no approval pauses, reels
...
WORK LIST
Every reel folder under books/medhavy/youtube that has a rendered master (<slug>.mp4 in the
...
1. VOICE SANITY. Open beat_sheet.json. Every beat must be engine "kokoro" with a
   Kokoro voice (am_onyx or af_bella). If ANY beat is engine "elevenlabs", SKIP the
   reel and log NEEDS-CONVERT (run the Medhavy converter first) — never call a paid engine.
...
5. RE-STAGE: copy the fresh master over the silent one in TMP/medhavy:
   cp <reel>/<slug>.mp4  books/TMP/medhavy/<the filename already there>.mp4
   (match the existing TMP/medhavy filename, which may carry a book__ prefix). Leave the
...
FINISH
Write ADD-SOUND-LOG-medhavy.md at books/TMP/medhavy: totals, fixed, skipped(has-audio),
