...
- **Renderer (RESOLVED):** the renderer is **Playwright/animated-deck**, canonical — NOT a coin-flip. The deck IS the animation (live D3, CSS keyframes, Solari flip-board, live-API panels); Playwright films the real browser, so all of it renders natively. Remotion is frame-based React (seek-to-frame); D3's imperative, time-driven animation (transitions, force sims, live data) has no per-frame seek and does not port without re-authoring every animation in React — lossy, and impossible for force/live-data pieces. So the Remotion `deck-lecture` path is NOT a substitute for animated D3 decks. animated-deck stays deterministic where the deck exposes a `window.__deckAnim.seek(id,p)` hook, and screenshot-holds the rest.
