#!/usr/bin/env python3
"""outro.py — the branded outro stage for explainer reels.
...
What it does (one command, rerunnable):
  ...
  3. composites over the ground with PIL text overlays (portable: works on
     drawtext-less ffmpeg builds, same trick as compile)
  ...
  Then rerun compile.py — only the outro slot recompiles.

Usage:
  python3 scripts/outro.py reels/<slug> [--beat B16] [--bears bearbrown]
...
def main():
    ...
    print(f"[outro] now: python3 scripts/compile.py {folder.name if not folder.is_absolute() else folder} --review")
