#!/usr/bin/env python3
"""convert.py — convert an existing video folder (bears-doodles / brownblue
physics format) into a explainer reel.
...
WHAT IS RE-PLANNED (the point of the conversion):
  - every visual. Old Manim scenes are NOT ported — each beat gets a shot
    ...
    archive queries). The human/agent rewrites visuals in the house grammar;
    the QC gates audit ONLY what survives conversion.

Usage:
  python3 scripts/convert.py physics/<slug> [--dest reels/<slug>-reel]
"""
...
def main():
    ...
    dest = (a.dest or Path("reels") / f"{src.name}-reel").resolve()
    if dest.exists():
    ...
    md = dict(ART_META)
    md.update(slug=f"{src.name}-reel", title=smd.get("title", src.name),
              ...
              purpose="CONVERTED from bears-doodles/brownblue format — every "
                      "visual re-planned in house grammar; audio carried over.")
    ...
    # scaffold for this reel's own Manim fragments (run picks it up)
    (dest / "scenes.py").write_text(
        '"""Per-reel Manim fragments — import the shared library and write\n'
        ...
        ".parents[2] / 'manim'))\n"
        "from animated_graphics import *   # noqa: F401,F403 — tokens + mobjects\n\n"
    ...
    lines += ["", "## Conversion contract",
              ...
              "- Old Manim scenes are NOT ported. Write Manim fragments in "
              "scenes.py for GRAPHIC beats; fill archive/ai slots via media/.",
              "- Run `bash scripts/run.sh <this reel>` — QC gates audit only "
    ...
    print(f"[convert] every beat is needs_review=true — retype in SHOTLIST.md, "
          f"then write scenes.py fragments")
