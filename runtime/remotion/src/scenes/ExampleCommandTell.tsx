...
/**
 * ExampleCommandTell — B05 — standard 5+5 teardown.
 */

export const exampleCommandTellSchema = z.object({
  sparkLine: z.string().default('Schema complete and $ARGUMENTS visible. Bash blast radius and $ARGUMENTS parsing: not shown.'),
});
export type ExampleCommandTellProps = z.infer<typeof exampleCommandTellSchema>;
...
const GETS_RIGHT = [
  'All five frontmatter fields with purpose — complete schema reference in four lines of YAML',
  '$ARGUMENTS shown in skill body — user input injection point visible without explanation needed',
  'argument-hint demonstrates the UX affordance: becomes syntax hint shown to users on invocation',
  'allowed-tools pre-approval stated with practical implication: reduces permission prompts',
  'Legacy format equivalence explicit: commands/<name>.md loads identically, only file path differs',
];

const BITES = [
  'No functional behavior: invoking it produces only the template — you can\'t watch it work',
  'model override lists haiku/sonnet/opus with no guidance on when to use which or cost tradeoffs',
  '$ARGUMENTS parsing entirely implicit — no example of split, optional handling, or defaults',
  'Bash in allowed-tools has full shell blast radius with no note on narrowing to minimal permissions',
  'No guidance on writing a good description for /help display — length, phrasing, truncation',
];

const CALLOUT = 'This skill is its own documentation — it demonstrates the format by being an example of the format. But it has no functional behavior: invoking it produces only the parse-perform-report template, not a working demonstration of any allowed tool.';

export const ExampleCommandTell: React.FC<ExampleCommandTellProps> = ({ sparkLine }) => {
