...
/**
 * HookifyTell — B05 — teardown: gets right + bites + callout.
 */

export const hookifyTellSchema = z.object({
  sparkLine: z.string().default('Format solid. Block action and stop/prompt condition fields: document them.'),
});
export type HookifyTellProps = z.infer<typeof hookifyTellSchema>;
...
const GETS_RIGHT = [
  'File location + naming explicit: hookify prefix, .local.md suffix, kebab-case verb-first name',
  'Two-tier rule design clearly distinguished: simple pattern vs. advanced conditions, each with examples',
  'Event type guide concrete: 4 events with working examples showing what they match',
  'Pattern pitfalls: too-broad, too-specific, YAML escaping — each with bad/good pair',
  'Message body guidance actionable: what detected · why problematic · suggest alternatives',
];

const BITES = [
  'block action: described but never demonstrated — what does Claude actually see when blocked?',
  'stop/prompt condition fields: available fields (stop_reason? user_prompt only?) undocumented',
  'Rule execution order: multiple matching rules — do all fire? in what order? not documented',
  'all event type: listed but no example, no performance note, no ordering implication',
  'conditions on stop/prompt: no complete example showing what fields these events expose',
];

const CALLOUT = 'The rule format is clean and the pitfall section is genuinely useful. What the skill leaves out: what actually happens at the tool layer when a rule blocks — and what condition fields stop and prompt events expose. Those gaps matter when writing rules that need to do more than match bash commands.';

export const HookifyTell: React.FC<HookifyTellProps> = ({ sparkLine }) => {
  ...
  return (
    <AbsoluteFill style={{ background: CLAUDE.PAGE }}>
      <div style={{
        ...
        WRITING HOOKIFY RULES · TEARDOWN
      </div>
      ...
      <div style={{
        ...
        Format solid. Block action and stop/prompt condition fields: document them.
      </div>
