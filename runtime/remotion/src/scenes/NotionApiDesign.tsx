...
export const notionApiDesignSchema = z.object({
  sparkLine: z.string().default('Sanity check first. DB → data source ID. Schema before filter. Sharing before debugging 404.'),
});
export type NotionApiDesignProps = z.infer<typeof notionApiDesignSchema>;
...
const WORKFLOW_STEPS = [
  { label: 'Sanity check first', note: 'GET /v1/users/me with Notion-Version · 400 missing_version = forgot header', color: '#4A7C59' },
  { label: 'DB → get data_source_id', note: 'schema lives on data source, not database · data_sources[].id from DB object', color: '#4A7C59' },
  { label: 'Schema before filter', note: 'filter conditions keyed by name AND type · select=equals, date=on_or_before', color: '#4A7C59' },
  { label: 'Sharing before debugging 404', note: '404 = almost always sharing, not bad ID · Connections → share with integration', color: '#4A7C59' },
];

const GOTCHAS = [
  { label: 'child_page / child_database', note: 'listed but not recursed · re-run notion_read_page.sh with child block ID', color: CLAUDE.SPARK },
  { label: 'File URLs expire ~1 hour', note: 're-fetch block for fresh URL · never cache block payload file URLs', color: CLAUDE.SPARK },
  { label: 'Pagination loop guard', note: 'error envelope instead of results → naive cursor loop runs forever', color: CLAUDE.SPARK },
  { label: 'Notion-Version on every request', note: 'helper function ensures it · raw curl outside helper silently misses it', color: CLAUDE.SPARK },
];

export const NotionApiDesign: React.FC<NotionApiDesignProps> = ({ sparkLine }) => {
