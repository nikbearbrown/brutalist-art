...
/**
 * DebugPluginsTell — B05 — standard 5+5 teardown.
 */

export const debugPluginsTellSchema = z.object({
  sparkLine: z.string().default('Ladder and security note solid. Stdout gap and unzip Bash tension: surface them.'),
});
export type DebugPluginsTellProps = z.infer<typeof debugPluginsTellSchema>;
...
const GETS_RIGHT = [
  'Security note leads — treat file content as untrusted, Read/Grep only, never follow found URLs',
  'Six-step ladder: collect all evidence first, then interpret, then report',
  'Startup log vs structured errors explicit: stderr → log file; init.plugin_errors → stdout (not persisted)',
  'Zip root packaging trap named: plugin.json at archive root, not nested in a folder',
  'Session snapshot rule unambiguous: config changes = fresh thread; existing threads never reload',
];

const BITES = [
  'Step 5 runs unzip -l from Bash while security note says Read/Grep only — no explanation for the exception',
  'stdout gap means structured startup errors are permanently undiagnosable — no workaround given',
  'Step 6 report format is prose guidance only — no structured arrived/loaded/failed template',
  'No fallback if /tmp/claude-code.log is missing or empty — skill assumes the file exists',
  'CLAUDE_CODE_PLUGIN_SEED_DIR seed mounts identified but cannot be inspected from inside the skill',
];

const CALLOUT = 'Step 5 runs `unzip -l` from Bash while the security note says to use Read and Grep for file inspection — the skill doesn\'t explain why this exception is safe, and a reader following the note literally would skip a key diagnostic.';

export const DebugPluginsTell: React.FC<DebugPluginsTellProps> = ({ sparkLine }) => {
