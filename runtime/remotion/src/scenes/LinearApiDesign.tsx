...
export const linearApiDesignSchema = z.object({
  sparkLine: z.string().default('UUID before mutate. Markdown body. Rate limit is HTTP 400 not 429. Epoch milliseconds.'),
});
export type LinearApiDesignProps = z.infer<typeof linearApiDesignSchema>;
...
const WORKFLOW_STEPS = [
  { label: 'Sanity check first', note: 'viewer query → id + name + email — confirms workspace is wired', color: '#4A7C59' },
  { label: 'UUID before mutate', note: 'discover team/state UUID with teams query before issueCreate/issueUpdate', color: '#4A7C59' },
  { label: 'Markdown body', note: 'description + comment = Markdown — not ADF, not plain text', color: '#4A7C59' },
  { label: 'Check success boolean', note: 'mutations return {success, issue} — verify success even after HTTP 200 + empty .errors', color: '#4A7C59' },
];

const GOTCHAS = [
  { label: 'Rate limit = HTTP 400', note: 'code RATELIMITED — 429-only retry loop silently misses Linear rate limits', color: CLAUDE.SPARK },
  { label: 'No totalCount', note: 'connections lack totalCount except search endpoints — paginate and count', color: CLAUDE.SPARK },
  { label: 'Epoch milliseconds', note: 'X-RateLimit-*-Reset headers in ms not seconds — divide by 1000 before sleeping', color: CLAUDE.SPARK },
  { label: 'UUID not identifier', note: 'passing ENG-123 to a mutation field → INVALID_INPUT with no clear UUID hint', color: CLAUDE.SPARK },
];

export const LinearApiDesign: React.FC<LinearApiDesignProps> = ({ sparkLine }) => {
