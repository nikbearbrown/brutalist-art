...
export const hookDevelopmentTellSchema = z.object({
  sparkLine: z.string().default('Prompt hooks are right for judgment. But they only fire on 4 of 9 events — unstated.'),
});
export type HookDevelopmentTellProps = z.infer<typeof hookDevelopmentTellSchema>;
...
const GETS_RIGHT = [
  'Prompt-Based recommended: correct — LLM judgment beats brittle regex logic in bash for open-ended checks',
  'CLAUDE_PLUGIN_ROOT in command paths — the right portability pattern for plugins that move machines',
  'Exit code semantics clear: 0 success, 2 blocking, other non-blocking — exact enough to reason from',
  'Parallel execution documented: no ordering, no shared output — prevents hook-sequencing bugs upfront',
  'Security patterns named: quote bash vars, validate inputs, deny path traversal, block sensitive files',
];

const BITES = [
  'Prompt hooks recommended without stating they only work on 4 of 9 events — PostToolUse silently skipped',
  'Hot-swap restriction buried: hooks load at session start; must restart claude — easy 20-minute debugging trap',
  'Parallel means no hook collaboration — one hook cannot see another\'s output; this implication is not spelled out',
  'Two config formats look similar: plugin hooks.json has description wrapper; settings does not — error is silent',
  'Matchers are case-sensitive: "Write" ≠ "write" — wrong case means the hook never fires, no error',
];

const CALLOUT = 'Prompt hooks are the recommended type — but they only work on PreToolUse, Stop, SubagentStop, and UserPromptSubmit. The other five events (PostToolUse, SessionStart, SessionEnd, PreCompact, Notification) only support Command hooks. A prompt hook on PostToolUse will silently do nothing.';

export const HookDevelopmentTell: React.FC<HookDevelopmentTellProps> = ({ sparkLine }) => {
