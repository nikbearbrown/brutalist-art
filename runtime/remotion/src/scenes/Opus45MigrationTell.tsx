...
/**
 * Opus45MigrationTell — B05 — teardown: 5 gets-right / 5 bites + callout.
 */

export const opus45MigrationTellSchema = z.object({
  sparkLine: z.string().default('Platform matrix and opt-in discipline solid. Azure gap and vague triggers: surface them.'),
});
export type Opus45MigrationTellProps = z.infer<typeof opus45MigrationTellSchema>;
...
const GETS_RIGHT = [
  'Platform matrix complete and paste-ready — 4 targets, 3 sources',
  'Haiku exclusion is explicit — "Do NOT migrate"',
  'context-1m beta header called out with a comment template',
  'Opt-in discipline prevents silent prompt breaking changes',
  '6-step workflow ends with a handoff: offer to help with issues',
];

const BITES = [
  'Azure AI Foundry: target string exists, no source strings in table',
  'Behavioral triggers vague — "reports tools called too frequently" not reproducible',
  '"Integrate thoughtfully" has no bad vs good placement example',
  'Effort parameter deferred to references/effort.md — no inline fallback',
  'No rollback guidance — only a vague offer to help if migration breaks',
];

const CALLOUT = 'Source model table covers 3 platforms, but the target table has 4 — Azure AI Foundry gets a target string but no source strings to replace.';

export const Opus45MigrationTell: React.FC<Opus45MigrationTellProps> = ({ sparkLine }) => {
