...
export const m5OnboardAnatomySchema = z.object({
  sparkLine: z.string().default('One orchestrator: detect, identify, flash, install. Button dance blocks the flash stage.'),
});
export type M5OnboardAnatomyProps = z.infer<typeof m5OnboardAnatomySchema>;
...
const STAGES = [
  { label: 'Detect + Identify', note: 'detect.py: enumerate ports, probe with esptool, cross-ref hardware signatures', color: '#4A7C59' },
  { label: 'Fetch firmware', note: 'M5Burner manifest API → UIFlow 2.0 binary, cached in ~/.cache/m5-onboard/', color: '#4A7C59' },
  { label: 'Flash', note: 'esptool write_flash — native-USB S3 needs physical BtnG0+BtnRST button dance', color: CLAUDE.SPARK },
  { label: 'Install apps', note: 'install_apps.py: paste-mode REPL upload → /flash/ · reboot via machine.reset', color: '#4A7C59' },
];

const RUN_PATTERN = [
  { label: 'onboard.py --apps buddy', note: 'single orchestrator covers all 4 stages — default for fresh Cardputer-Adv', color: '#4A7C59' },
  { label: 'Run in background + tee', note: '2-3 min foreground run looks like a hang — stream log + Monitor', color: CLAUDE.SPARK },
  { label: 'smoke_test.py', note: 'I2C + LCD + speaker + button check after provisioning', color: '#4A7C59' },
];

export const M5OnboardAnatomy: React.FC<M5OnboardAnatomyProps> = ({ sparkLine }) => {
  ...
  const runSprings = RUN_PATTERN.map((_, i) =>
    spring({ frame: frame - 18 - i * 10, fps, config: { damping: 28, stiffness: 120, mass: 0.85 } })
  );
    ...
    <AbsoluteFill style={{ background: CLAUDE.PAGE }}>
      ...
      <div style={{ position: 'absolute', left: W * 0.04, top: COL_TOP, width: LEFT_W }}>
        <div style={{
          ...
          opacity: clamp(stageSprings[0], 0, 1),
        }}>
          STAGES
        </div>
        {STAGES.map((item, i) => {
      ...
      <div style={{ position: 'absolute', left: W * 0.54, top: COL_TOP, width: RIGHT_W }}>
        <div style={{
          fontFamily: SANS, fontSize: 10, fontWeight: 700, letterSpacing: 3,
          color: '#4A7C59', textTransform: 'uppercase' as const, marginBottom: 8,
          opacity: clamp(runSprings[0], 0, 1),
        }}>
          RUN PATTERN
        </div>
        {RUN_PATTERN.map((item, i) => {
          const op = clamp(runSprings[i], 0, 1);
          const isWarn = item.color === CLAUDE.SPARK;
          return (
            <div key={i} style={{
              background: isWarn ? 'rgba(217,119,87,0.05)' : '#FFFFFF',
