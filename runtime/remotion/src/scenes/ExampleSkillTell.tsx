...
/**
 * ExampleSkillTell — B05 — standard 5+5 teardown.
 */

export const exampleSkillTellSchema = z.object({
  sparkLine: z.string().default('Three-mode distinction and description trigger solid. Trigger mechanism and testing: unspecified.'),
});
export type ExampleSkillTellProps = z.infer<typeof exampleSkillTellSchema>;
...
const GETS_RIGHT = [
  'Skills vs commands vs agents named explicitly — right abstraction choice before building',
  'Description-as-trigger with concrete good patterns: phrases, keywords, topic areas',
  'Five-point skill content guidelines: purpose, when, structure, actionable, examples',
  'Optional directory structure documented: references, examples, scripts — clear upgrade path',
  'Overlap avoidance named as a best practice — a non-obvious risk for multi-skill plugins',
];

const BITES = [
  'version and license frontmatter fields documented but runtime effect is never explained',
  'Trigger mechanism unspecified: no guidance on phrase matching vs topic similarity vs embedding',
  '"Test that the skill activates" is advice without a methodology, tooling, or example',
  'No description length guidance — longer vs shorter descriptions, effect on match quality',
  'Overlap avoidance listed as best practice but no detection method or resolution process',
];

const CALLOUT = 'The description field is both the trigger and the documentation — it tells Claude when to activate, but the mechanism is implicit. There\'s no guidance on how Claude resolves ambiguity when two skills\' descriptions both match.';

export const ExampleSkillTell: React.FC<ExampleSkillTellProps> = ({ sparkLine }) => {
