...
/**
 * ClaudeMdImproverTell — B05 — teardown: 5 gets-right / 5 bites + callout.
 */

export const claudeMdImproverTellSchema = z.object({
  sparkLine: z.string().default('Report gate and diff-with-why solid. head-50 cap and unattended gap: surface them.'),
});
export type ClaudeMdImproverTellProps = z.infer<typeof claudeMdImproverTellSchema>;
...
const GETS_RIGHT = [
  'Five-location taxonomy covers the full CLAUDE.md surface',
  '"Quality report before updates" is a hard, explicit gate',
  'Six-criterion rubric is concrete — scored and A-F graded',
  'Diff-with-why format makes proposals reviewable before apply',
  '# shortcut and .claude.local.md tips are genuinely non-obvious',
];

const BITES = [
  'Phase 3 confirmation has no guidance for unattended sessions',
  'Weight qualifiers (High/Medium) — numeric scores unclear in sum',
  'Template reference is external — no fallback if file is missing',
  'head -50 cap silently truncates large monorepos (>50 files)',
  '"Preserve existing structure" — no guidance when structure IS the problem',
];

const CALLOUT = 'The quality report gate (Phase 3 before Phase 4) is the discipline that prevents silent rewrites — but the skill gives no guidance on what to do when running unattended.';

export const ClaudeMdImproverTell: React.FC<ClaudeMdImproverTellProps> = ({ sparkLine }) => {
