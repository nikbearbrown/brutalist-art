...
/**
 * DatadogApiTell — B05 — standard 5+5 teardown.
 */

export const datadogApiTellSchema = z.object({
  sparkLine: z.string().default('Site check, v1/v2 routing, and pagination solid. Spans envelope and dashboard PUT trap: surface them.'),
});
export type DatadogApiTellProps = z.infer<typeof datadogApiTellSchema>;
...
const GETS_RIGHT = [
  'Security note leads — prompt injection first, before any operation',
  'v1/v2 split explicit by resource type, not by "use the newer one"',
  'Regional site sanity-check: validate endpoint called before real work',
  'globoff called out with exact failure symptom: "bad range in URL"',
  'Three pagination schemes named and assigned to specific endpoints',
];

const BITES = [
  'Only dd_logs.sh bundled — all other ops are raw curl with no write helpers',
  'Dashboard PUT-replaces-all warning is a single sentence, easy to miss',
  'Double .attributes on events (data[].attributes.attributes.title) not flagged as a trap',
  'Spans vs logs JSON:API asymmetry appears as a note, not a prominent callout',
  'date command GNU/BSD difference documented only in a code comment',
];

const CALLOUT = 'The spans endpoint follows the full JSON:API envelope; the logs endpoint does not — same v2, different wrapper, and the 400 that results says "malformed body" without naming the missing field.';

export const DatadogApiTell: React.FC<DatadogApiTellProps> = ({ sparkLine }) => {
