...
/**
 * BuildMcpAppTell — B05 — teardown: gets right / bites + callout.
 */

export const buildMcpAppTellSchema = z.object({
  sparkLine: z.string().default('Two-part reg and bundle-inlining documented. Cache flush and CSP blank: surface them earlier.'),
});
export type BuildMcpAppTellProps = z.infer<typeof buildMcpAppTellSchema>;
...
const GETS_RIGHT = [
  'Widget-vs-elicitation routing table explicit before any widget code',
  'Two-part registration documented with working TypeScript (tool + resource)',
  'Bundle inlining called out as mandatory with the rewrite snippet',
  'App class API complete: 8 methods with direction and when to use each',
  'Design rules concrete: one widget per tool · description mentions · degradation auto',
];

const BITES = [
  'Desktop cache flush (full quit, not window close) buried in testing section',
  'CSP blank rectangle with no console error — documented in reference file, not inline',
  'frameDomains restriction in Claude mentioned in a footnote-sized table cell',
  'Wrong MIME = blank widget — no explicit error path documented alongside it',
  'sendMessage vs updateModelContext distinction after method list, not at decision point',
];

export const BuildMcpAppTell: React.FC<BuildMcpAppTellProps> = ({ sparkLine }) => {
  ...
  return (
    <AbsoluteFill style={{ background: CLAUDE.PAGE }}>
      ...
      <div style={{
        ...
        Two-part reg and bundle-inlining documented.<br />Cache flush and CSP blank: surface them earlier.
      </div>
      ...
      <div style={{
        ...
        <div style={{ fontFamily: SANS, fontSize: 11, color: CLAUDE.INK }}>
          <span style={{ fontWeight: 700 }}>Two silent failures, same symptom:</span>
          {' wrong MIME type and missing bundle inlining both produce a blank rectangle with no error in the main console.'}
        </div>
      </div>
