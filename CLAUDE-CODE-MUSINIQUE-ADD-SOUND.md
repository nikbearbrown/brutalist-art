# Claude Code Prompt — Add Sound to Silent Musinique Videos (Kokoro, free)

The Musinique videos rendered SILENT — their narration mp3s were never generated, so the
audio-first pipeline composited silence. This generates the free Kokoro narration
(the reel's assigned Musinique voice — Liam `am_onyx` or Puck am_puck), re-renders at 4K so the
sound composites in, and overwrites the silent copy in `TMP/musinique`. Unattended, FREE
...
```text
Add sound to every silent Musinique video. Unattended batch: no approval pauses, reels
...
WORK LIST
Every reel folder under books/musinique/youtube that has a rendered master (<slug>.mp4 in the
...
1. VOICE SANITY. Open beat_sheet.json. Every beat must be engine "kokoro" with a
   Kokoro voice (am_onyx or am_puck). If ANY beat is engine "elevenlabs", SKIP the
   reel and log NEEDS-CONVERT (run the Musinique converter first) — never call a paid engine.
...
5. RE-STAGE: copy the fresh master over the silent one in TMP/musinique:
   cp <reel>/<slug>.mp4  books/TMP/musinique/<the filename already there>.mp4
   (match the existing TMP/musinique filename, which may carry a book__ prefix). Leave the
...
FINISH
Write ADD-SOUND-LOG-musinique.md at books/TMP/musinique: totals, fixed, skipped(has-audio),
