...
description: >
  ...
  (Onda terminal in, vox output). Subject-adaptive: technical/quantitative books
...
as candidate cards. A CLI video shows code (or research prompts) typed into a
terminal and shows the **output** as a vox-explainer beat. Your job is to spot the
...
- **RESEARCH lane** (biology, history, humanities, law, policy, medicine-as-
  ...
  terminal shows the research prompts; the output is the synthesized artifact (a
...
- Prompt seed: the actual `claude "…"` ASK — the command typed into the terminal.
