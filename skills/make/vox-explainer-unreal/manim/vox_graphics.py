...
BLUE = "#5B7B9C"; TERRA = "#D35F43"
GOLD = "#F5D061"; SLATE = "#3E5559"
SERIF = "Georgia"  # swap for EB Garamond if installed
...
class SerifLabel(VGroup):
    ...
    def __init__(self, text, accent=BLUE, size=30, **kw):
        super().__init__(**kw)
        t = Text(text, font=SERIF, color=INK, font_size=size, weight=BOLD)
...
class LabelChip(VGroup):
    """Accent block, white serif text ('Three-fifths clause')."""

    def __init__(self, text, accent=CRIMSON, size=26, **kw):
        super().__init__(**kw)
        t = Text(text, font=SERIF, color=WHITE, font_size=size)
