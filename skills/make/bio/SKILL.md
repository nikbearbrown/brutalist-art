...
## Length (Phase 2 refactor)

`bio` (formerly `mini-bio`; `voxbio` merged in — its editorial-collage treatment is described
in `reference/collage-style.md`) takes a length parameter, **default 3:00**. Ask for a shorter
or longer bio and the beat count scales; "mini-bio" now just means asking for a short one.
Visual treatment is chosen per beat like any other reel — whatever tool fits, composited at the end.
