...
SKINS = {
    ...
    # "voxpaper" REMOVED (2026-07-06, by user order). Vox explainers are a
    # PRODUCTION COMPOSITING PIPELINE with no previz stage — the first
    # watchable pass is `bash scripts/vox_run.sh reels/<slug>` (slates in
    # unfilled slots). See aspects/explainer/vox-explainer/SKILL.md.
    # Do not re-add this skin.
}
...
def main():
    ...
    ap.add_argument("--skin", default="journal",
                    help="journal = scrapbook previz (doodle/brownblue only)")
    args = ap.parse_args()
    if args.skin == "voxpaper":
        sys.exit("voxpaper was REMOVED: vox explainers have NO previz stage.\n"
                 "First watchable pass:  bash scripts/vox_run.sh reels/<slug>\n"
                 "See aspects/explainer/vox-explainer/SKILL.md.")
    if args.skin not in SKINS:
        sys.exit(f"unknown skin '{args.skin}' (available: {', '.join(sorted(SKINS))})")
