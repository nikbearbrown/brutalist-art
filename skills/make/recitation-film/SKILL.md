...
## Stock plates: generate by default, hunt only the real thing

Most slots in a literature film are GENERIC period scenes — nano-banana (via
Higgsfield) or Midjourney mints them as stock. The plan emits
`STILL_PROMPTS.md`: one line per slot, VERY detailed (period, medium, light,
composition, texture), beat id first, aspect flag last —
`B43, archival photograph, tea table spread — tiered cake stands with iced
fancies, ices melting in coupe glasses… --ar 16:9`. No lyric text or
lettering inside any plate. These slots are `source: ai` (disclosure sidecar
at pantry intake).

The exceptions are TRUE-ARCHIVE slots, where the real object IS the
teaching: the first-printing scan, the actual Rembrandt etching, a genuine
playbill, a named painting. Generating a fake one would be a scholarship-gate
violation — a forged citation, not a plate. Those keep `source: archive`,
FIND links, and mandatory sidecars. Rule of thumb: if a teach chip or
DOCUMENT beat points AT the object, hunt it; if the plate merely stages a
scene, generate it.
