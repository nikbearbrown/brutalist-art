...
**The beat ledger (`todo.json` per video — Gate-2 addition):** the system logs, per reel, which
beats still need filling and **how**. Every beat carries a status (`filled` / `needs-fill`) and a
fill plan (`manim` | `remotion` | `ai-video-prompt` | `historical-image` | `user-capture`), plus
the suggested prompt where one applies. This makes the fill loop queryable and agent-drivable:
Claude Code can ask "which beats suggest `animated_graphics.py`?" (`art todo <reel>
--method animated_graphics`), try to fill each one, and — when it judges a prompt-based clip or
an **actual historical image** better than an animation — render the request-card placeholder
instead and **update that video's todo** with the new plan and prompt. Human-readable mirror in
`STATUS.md` (the shape already present in the explainer example); `compile.py` regenerates the
ledger on every pass so it never drifts from the actual clips on disk.
...
| **3** | Vendor runtime per §5 under the §2b names (vox canonical, renamed on arrival, references swept per commit); path-hygiene sweep (§5.9); shared-lib dedup (§3.5); `art` dispatcher + shared `--help` + shared schema; `animated_graphics.py` decide-or-request behavior + the per-video beat ledger (`todo.json` + `STATUS.md`, regenerated by `compile.py`). Skills run in isolation. | "Phase 3: vendored runtime + unified grammar" | Run one skill end-to-end from a fresh clone (no `../` reach-outs); `art todo` shows an accurate fill list |
...
| **4.5 (P0 fixes, from Gate-4 scan)** | Fix the Phase-3 vendoring regressions the cross-repo scan found (see `TODO.md` Theme 1): repoint `examples/*/scenes.py` at `runtime/manim`; add `PROMPTS.md` / exempt zero-open reels for GATE F; fix-or-disable the QC gates; author `figure-planner/SKILL.md`; **build `./art fill-in`** (auto-render every pipeline-makeable beat so cards are only ever for human beats); sweep the ~55 stale `aspects/…` paths; reconcile the explainer skill-local graphics copy; `LICENSE` (MIT). | "Phase 4.5: close P0 regressions + fill-in" | `./art run` works on the shipped examples; `./art fill-in` leaves cards only for human beats |
| **5 (reframed — META EXAMPLE VIDEOS)** | The examples become the **"Brutalist — Claude for Video Production"** playlist: one worked example **per video type**, each a video *about the toolkit itself*, rebuildable from its own inputs. `README.md` is the script for the intro video *"What is Brutalist?"* (Claude does the technical build; humans own judgment/aesthetics/original ideas). Bring the **lecture pipeline** in (D4 reversed — 34 lecture videos shipped): vendor `deck-lecture` + `lecture`/`silent_run.py` as first-class skills (`lecture-video` + `deck-video`, names TBD) with one restored lecture as its example. Per-example rebuild READMEs; `LEARN.md` ladder; verify no-key examples rebuild. | "Phase 5: meta example videos + lecture pipeline" | Human rebuilds the no-key + ElevenLabs examples; approves the intro-video script |
| **6** | `CONTRIBUTING.md` (skill folder contract: `SKILL.md` + `README.md` + scripts, one shape); the Brutalist governance layer scope decision (`TODO.md` Theme 4); run the brief's §6 acceptance test in a clean container; fix gaps; publish to the remote if approved | "Phase 6: front door + acceptance" | Final acceptance sign-off |

**Phase 5 reframe & lecture reversal (Gate-4 feedback), recorded here:**
- Examples are **meta**: each is a video in the *"Brutalist — Claude for Video Production"* playlist, made by the toolkit about the toolkit. The intro video is *"What is Brutalist?"*, and `README.md` is its script (already rewritten).
- **D4 is reversed.** The lecture pipeline (chapter → HTML `.dc.html` deck → narrated lecture video, idea-karaoke timing) has **34 finished videos** (`cancer-nanomedicine` 12, `cancer-medicine` 11, `cancer-research` 11) and a full source in `unreal-reels/skills/{deck-lecture,lecture-assets}` + `silent_run.py`. It becomes first-class, not out-of-scope.
- **Fill-in** (`./art fill-in`) is a new first-class command: audit the beat sheet and render every beat the machine can make (Manim/Remotion), so request cards are only ever left for genuinely human beats. Built in Phase 4.5.
- **Lecture skill name (RESOLVED at gate):** **`deck-lecture`** — one skill, chosen because the name conveys it creates an HTML **deck** AND a **lecture** (video). It covers the whole pipeline (chapter → HTML deck → narrated lecture video) and can enter at the deck stage if you already have a `.dc.html` deck. Vendors `skills/animated-deck/` (the proven Playwright/D3/idea-karaoke pipeline behind the 34 videos); the session `lecture` name resolves to it via alias. **Renderer is Playwright, canonical (not Remotion):** the deck IS the animation (live D3/CSS/Solari/live-API), and D3's time-driven model doesn't port to Remotion's frame-based React without a lossy re-author — Playwright films the live browser, so it all renders natively.
- **Lecture-pipeline CORRECTION (live-verified 2026-07-12):** the process that actually made the 34 lecture videos is **`skills/animated-deck/`** (D3 `deck.html` + Playwright headless-Chromium render + faster-whisper idea-karaoke timing + `batch_idea_highlight.py`), NOT the Remotion `deck-lecture`. Phase 5 vendors `animated-deck`; this adds Playwright + Chromium as deps and a slide-schema (`S01..Snn`) deck beat_sheet distinct from the vox `B01..Bnn` beat_sheet. See `TODO.md` Theme 3 lead item.
- **Reconciliation status:** only the `ch-lecture` and `vox/youtube` pipelines are live-verified end-to-end; the other video types were built by now-terminated sessions and rest on the (byte-identical) vendored source skills until each is run once in Phase 5. The vox QC gates (`static_scene_check`/`wcag_margin_check`/`manim_layout_audit`) are now vendored to `runtime/qc/`; the `bearbrown` outro is brand MEDIA, supplied via `ART_BEARBROWN`, not committed.
