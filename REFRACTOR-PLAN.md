...
| **5 (BUILD-DRIVEN — the toolkit tests itself)** | Merges the old 4.5 + 5. Build a real video of **every type** with Claude Code — the meta "Brutalist — Claude for Video Production" playlist, first = **"What is Brutalist?"** — in key-ladder order (no-key first). Each build LOGS everything (`youtube/<slug>/BUILD-LOG.md`); every breakage it hits is FIXED in the toolkit and committed (the P0 regressions — dead `scenes.py` paths, GATE F, `fill-in`, `figure-planner` SKILL.md, LICENSE, Manim palette gaps — surface here as build failures, not guesses). The finished folders ship as the CAPABILITIES/LEARN examples. Vendor `animated-deck` when `deck-lecture` (#15) is built. See `EXAMPLES-CAMPAIGN.md`. | per-fix commits + per-example folders | Human reviews each built video; approves the intro-video script; signs off tier by tier |
